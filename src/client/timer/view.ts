export class View {
  ...
  public subscribeWhenApplyTimeIsRequested(
    handler: (minutes: number, seconds: number) => void,
  ): void {
    document
  ...
  public showTime(minutes: number, seconds: number): void {
    ...
    if (htmlElementMinutes)
      htmlElementMinutes.innerHTML = getTwoDigitsNumber(minutes);
    if (htmlElementSeconds)
      htmlElementSeconds.innerHTML = getTwoDigitsNumber(seconds);
